...
//sys getNamedPipeInfo(handle syscall.Handle, flags *uint32, outBufferSize *uint32, inBufferSize *uint32, maxInstances *uint32) (err error) = GetNamedPipeInfo
//sys getNamedPipeHandleState(handle syscall.Handle, state *uint32, curInstances *uint32, maxCollectionCount *uint32, collectDataTimeout *uint32, userName *uint32, nMaxUserNameSize uint32) (err error) = GetNamedPipeHandleState
//sys getNamedPipeServerProcessId(handle syscall.Handle, serverProcessId *uint32) (err error) = GetNamedPipeServerProcessId
...
func (l *PipeListener) File() (*os.File, error) {
	var s syscall.Handle
	p, _ := syscall.GetCurrentProcess()
	err := syscall.DuplicateHandle(p, l.handle, p, &s, 0, true, syscall.DUPLICATE_SAME_ACCESS)
	if err != nil {
		return nil, err
	}
	return os.NewFile(uintptr(s), l.addr.String()), nil
}

func FileListener(f *os.File) (net.Listener, error) {
	err := getNamedPipeInfo(syscall.Handle(f.Fd()), nil, nil, nil, nil)
	if err != nil {
		return nil, err
	}
	return &PipeListener{
		addr:   PipeAddr(f.Name()),
