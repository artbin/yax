...
var (
	...
	procGetNamedPipeHandleState     = modkernel32.NewProc("GetNamedPipeHandleState")
...
func getNamedPipeInfo(handle syscall.Handle, flags *uint32, outBufferSize *uint32, inBufferSize *uint32, maxInstances *uint32) (err error) {
	r1, _, e1 := syscall.Syscall6(procGetNamedPipeInfo.Addr(), 5, uintptr(handle), uintptr(unsafe.Pointer(flags)), uintptr(unsafe.Pointer(outBufferSize)), uintptr(unsafe.Pointer(inBufferSize)), uintptr(unsafe.Pointer(maxInstances)), 0)
	if r1 == 0 {
		if e1 != 0 {
			err = error(e1)
		} else {
...
func getNamedPipeHandleState(handle syscall.Handle, state *uint32, curInstances *uint32, maxCollectionCount *uint32, collectDataTimeout *uint32, userName *uint32, nMaxUserNameSize uint32) (err error) {
	r1, _, e1 := syscall.Syscall9(procGetNamedPipeHandleState.Addr(), 7, uintptr(handle), uintptr(unsafe.Pointer(state)), uintptr(unsafe.Pointer(curInstances)), uintptr(unsafe.Pointer(maxCollectionCount)), uintptr(unsafe.Pointer(collectDataTimeout)), uintptr(unsafe.Pointer(userName)), uintptr(nMaxUserNameSize), 0, 0)
	if r1 == 0 {
		if e1 != 0 {
			err = error(e1)
		} else {
...
func getNamedPipeServerProcessId(handle syscall.Handle, serverProcessId *uint32) (err error) {
	r1, _, e1 := syscall.Syscall(procGetNamedPipeServerProcessId.Addr(), 2, uintptr(handle), uintptr(unsafe.Pointer(serverProcessId)), 0)
	if r1 == 0 {
		if e1 != 0 {
			err = error(e1)
		} else {
